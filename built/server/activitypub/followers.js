...
const _default = async (ctx)=>{
    ...
    const limit = 10;
    const partOf = `${_config.default.url}/users/${userId}/followers`;
    if (page) {
        const query = {
            followeeId: user._id
        };
        ...
        const renderedFollowers = await Promise.all(followings.map((following)=>(0, _followuser.default)(following.followerId)));
        const rendered = (0, _orderedcollectionpage.default)(`${partOf}?${_url.query({
            page: 'true',
            cursor
        })}`, user.followersCount, renderedFollowers, partOf, null, inStock ? `${partOf}?${_url.query({
            page: 'true',
        ...
        // index page
        const rendered = (0, _orderedcollection.default)(partOf, user.followersCount, user.hideFollows ? null : `${partOf}?page=true`, null);
        ctx.body = (0, _renderer.renderActivity)(rendered);
