...
let _class = class _class extends _channel.default {
    ...
    async onNote(note) {
        const noteTags = note.tags ? note.tags.map((t)=>t.toLowerCase()) : [];
        const matched = this.q.some((tags)=>tags.every((tag)=>noteTags.includes((0, _normalizetag.normalizeTag)(tag))));
        if (!matched) return;
        // Renoteなら再pack
    ...
    constructor(...args){
        super(...args);
        _define_property(this, "chName", 'hashtag');
        _define_property(this, "q", null);
    }
};
_define_property(_class, "shouldShare", false);
